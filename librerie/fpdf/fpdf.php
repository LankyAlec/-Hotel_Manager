...
{
	// Logo Rita
		if($GLOBALS['page']!="fattura"){$this->Image('img/logo_rita.jpg',15,13,650);}
		//template Rita
		$this->Image('img/template.jpg',15,35,40,245);
					
		if($GLOBALS['page']!="analisi" && $GLOBALS['page']!="giustificativo" && $GLOBALS['page']!="fattura" && $GLOBALS['page']!="integratori")
			{
				$this->SetFillColor($color_box);
				//rettangolo ascissa, ordinata, larghezza, altezza, F=riempito, D=contornato
				$this->RoundedRect(155, 25, 40, 7, '2', '24', 'DF');
				//set cell
				$this->SetFont('Arial','B',12);
				$this->SetTextColor($color_pasto_r, $color_pasto_g, $color_pasto_b);
				$this->SetY(27.5);
				$this->SetX(163.5);
				$this->Cell(23, 3, $_POST[modello], 0 , 1, 'C');
				$this->SetFont('Arial','B',5);	
				$this->SetTextColor($color_var_pasti_r, $color_var_pasti_g, $color_var_pasti_b);
				$this->SetY(30);
				$this->SetX(164);
				$this->Cell(23, 7, 'CODICE DIETA', 0, 1, 'C');
			}

			//set line
			$this->SetLineWidth(0.2);
			if($GLOBALS['page']!="fattura"){$this->Line($this->lMargin, 35, 210 - $this->rMargin, 35);}
...
function Footer()
{
	...
	$this->SetTextColor($color_pasto_r, $color_pasto_g, $color_pasto_b);
		...
		$this->SetY(-15);
		$this->SetX(26);
		$this->Cell(70,10, $page.'Via Lazzaretto 25, presso BSL Medica - ACIREALE. Cell. 3923479562',0,0,'C');
		

	if($GLOBALS['page']!="analisi" && $GLOBALS['page']!="giustificativo" && $GLOBALS['page']!="fattura")
	{
		// Page number
		$this->SetX(176);
		$this->Cell(20,10,'Pagina '.$this->PageNo(),0,0,'R');
	}
}
...
function contamulticell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	// Output text with automatic or explicit line breaks
	if(!isset($this->CurrentFont))
		$this->Error('No font has been set');
...
class PDF extends FPDF
	{	
		...
		function Footer()
		{
			//queste variabili anche se definite prima bisogna ridefinirle
			$color_pasto_r="14";
			$color_pasto_g="179";
			$color_pasto_b="238";
			// Position at 1.5 cm from bottom
