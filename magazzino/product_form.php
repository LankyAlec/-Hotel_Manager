...
<div class="d-flex align-items-center justify-content-between mb-3">
  ...
  <a class="btn btn-outline-secondary" href="magazzino.php">← Indietro</a>
</div>
...
(function () {
  ...
  function toggleCaricoFieldsNew() {
    ...
    document.querySelectorAll('.of-only-scarico-new')
      .forEach(el => el.classList.toggle('d-none', isCarico));
    if (isCarico) {
      const d = document.getElementById('new_mov_destinazione');
      if (d) d.value = '';
    }
  ...
  function toggleCaricoFieldsEdit() {
    ...
    document.querySelectorAll('.of-only-scarico-edit')
      .forEach(el => el.classList.toggle('d-none', isCarico));
    if (isCarico) {
      const d = document.getElementById('edit_destinazione');
      if (d) d.value = '';
    }
  }
...
lines.push(`<div><b>Note:</b> ${showOrDash(note)}</div>`);
  ...
  if (box) {
    ...
    blocks.push(rowHtml('Operatore', showOrDash(op)));
    if (tipo === 'SCARICO') {
      blocks.push(rowHtml('Destinazione', showOrDash(dest)));
    }
