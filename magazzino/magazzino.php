...
<div class="card toolbar-card mb-3 border-0 shadow-sm">
  <div class="card-body">
    <form id="filterForm" class="row g-3 align-items-end" method="get" action="magazzino.php">

      <input type="hidden" name="hz" value="1">
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-2">
  ...
  <button type="button" class="btn btn-primary" style="width:163.8px;" data-bs-toggle="modal" data-bs-target="#scarichiModal">
    <i class="bi bi-box-arrow-up-right"></i> Scarichi
  </button>
...
<div class="modal fade" id="scarichiModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Scarichi</h5>
      ...
      <div class="modal-body">
        <div class="row g-2 align-items-end mb-3">
          <div class="col-12 col-md-3">
            ...
            <input type="date" class="form-control" id="sc_from">
          </div>
          <div class="col-12 col-md-3">
            <label class="form-label">Al</label>
            <input type="date" class="form-control" id="sc_to">
          </div>
          ...
          <div class="col-12 col-md-2 d-flex gap-2">
            <button type="button" class="btn btn-primary w-100" id="sc_btn_filter">
              <i class="bi bi-search"></i>
            </button>
            <button type="button" class="btn btn-success w-100" id="sc_btn_csv" title="Scarica CSV">
              <i class="bi bi-download"></i>
            </button>
          </div>
        </div>

        <div id="scarichiWrap" class="border rounded p-3">
          <div class="text-secondary">Imposta i filtri e premi cerca.</div>
        </div>
      </div>
...
<style>
  ...
  .of-exp-warn { background: rgba(255,193,7,.22);   color:#b58100; }
...
(() => {
  ...
  async function refresh() {
    ...
    try {
      ...
      const newUrl = 'magazzino.php?' + qs.toString();
...
(function(){
  const wrap = document.getElementById('scarichiWrap');
  const from = document.getElementById('sc_from');
  const to   = document.getElementById('sc_to');
  const dest = document.getElementById('sc_dest');
  const btnF = document.getElementById('sc_btn_filter');
  const btnC = document.getElementById('sc_btn_csv');

  async function loadScarichi(){
    if (!wrap) return;
    wrap.innerHTML = '<div class="text-secondary">Caricamento...</div>';
    const qs = new URLSearchParams({
      from: from?.value || '',
      to: to?.value || '',
      dest: dest?.value || ''
    });
    const r = await fetch('scarichi_ajax.php?' + qs.toString(), { headers: { 'Accept':'application/json' }});
    const j = await r.json();
    wrap.innerHTML = j.ok ? j.html : (j.html || 'Errore');
  }

  btnF?.addEventListener('click', loadScarichi);
  ...
  btnC?.addEventListener('click', () => {
    ...
    window.location.href = 'scarichi_export_csv.php?' + qs.toString();
  });
})();
